...
% removed small map
% LAS 2024-10-20: days of C4 cluster
% LAS 2024-10-25: Revised TRACK 2023S solutions now using consistent TRACK
% parameter choices
...
axe1 = axes('Position',[0.08 0.810 0.855 0.182],'Box','on','NextPlot','add','XTickLabels',[]);
axe2 = axes('Position',[0.08 0.620 0.855 0.182],'Box','on','NextPlot','add','XTickLabels',[]);
axe3 = axes('Position',[0.08 0.425 0.855 0.182],'Box','on','NextPlot','add','XTickLabels',[]);
axe4 = axes('Position',[0.08 0.230 0.855 0.182],'Box','on','NextPlot','add','XTickLabels',[]);
axe5 = axes('Position',[0.08 0.035 0.855 0.182],'Box','on','NextPlot','add');
...
text(lake_dates(1)+0.25, y_plot_cdot(1)+0.35, {'L1A';'L1B'},'FontName','AvenirBold','FontSize',fontsize+1,...
    'FontAngle','italic','HorizontalAlignment','left','Rotation',0); 
text(lake_dates(end)+0.05, y_plot_cdot(1)+0.35, {'C5';'North'},'FontName','AvenirBold','FontSize',fontsize+1,...
    'FontAngle','italic','HorizontalAlignment','left','Rotation',0); 
...
text(t_plot(1)+0.15,-0.035,'L1A, L1B filling...',...
...
text(t_plot(1)+3.5, y_plot_cdot(2)-0.20, 'SQ35','FontName','AvenirBold','FontSize',fontsize,'Color',[0.4940    0.1840    0.5560],'HorizontalAlignment','center');
text(t_plot(1)+4.5, y_plot_cdot(2)-0.20, 'SQ36','FontName','AvenirBold','FontSize',fontsize,'Color',[0.4660    0.6740    0.1880],'HorizontalAlignment','center');
text(t_plot(1)+5.5, y_plot_cdot(2)-0.20, 'SQ37','FontName','AvenirBold','FontSize',fontsize,'Color',[0.3010  0.7450  0.9330],'HorizontalAlignment','center');
...
text(lake_fill_dates(4)+0.15,-0.035,'L3A, L3B filling...',...
         'FontName','AvenirBold','FontSize',fontsize+1,'FontAngle','italic','HorizontalAlignment','left')
...
xlim([t_plot(1) t_plot(2)]); ylim([y_plot_strain(1) y_plot_strain(2)])
ylabel('$\dot{\epsilon}_{lon}(t)-\dot{\epsilon}_{lon}(t_{165})$  [ yr$^{-1}$ ]','Interpreter','latex','FontSize',fontsize+1)
legend(['SQ31' char(8211) '37'],['SQ32' char(8211) '34'],['SQ34' char(8211) '37'],['SQ35' char(8211) '36'],'Location','NorthWest','NumColumns',4,...
            'Location','West','EdgeColor','none','color','none')
set(gca,'FontName','Avenir','tickdir','in','LineWidth',0.6,'FontSize',fontsize); 
set(gca,'ytick',y_plot_strain(1)+y_plot_strain_tick:y_plot_strain_tick:y_plot_strain(2))
...
%% print figure
print(gcf,'-dpng','-r300',sprintf('../../paperfigs/suppfig4.2_C4_cdot_distillations_260119.png')); 

%% 165-230 averages of c_dot_err
for i = 1:22; xxx(i,1) = nanmean(daily_epsilon_zz(i).c_dot_delta_t_combo_err); end

% check figure: do errors in c_dot vary across array? (answer: yes)
figure; clf; 
plot(1:1:22,xxx,'^-'); hold on
...
xlabel('Station ID'); title('2023/165–230')
set(gca,'xticklabel',upper(station_names_short),'xtick',1:1:num_sta-1); 
grid on; xlim([0 23])
% print figure
%print(gcf,'-dpng','-r300',sprintf('C4/sigma1_c_dot_2023_165_230_260121.png')); 

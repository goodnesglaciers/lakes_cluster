...
% removed small map
% LAS 2024-10-20: days of C5 cluster
...
text(lake_dates(end)+0.50, y_plot_cdot(1)+0.40, {'C5';'Northern'},'FontName','AvenirBold','FontSize',fontsize+1,...
...
text(lake_dates(2)+0.50, y_plot_strain(1)+0.015, {'C5';'Southern'},'FontName','AvenirBold','FontSize',fontsize+1,...
    'FontAngle','italic','HorizontalAlignment','center','Rotation',0); 
...
for i=11
    plot(daily_epsilon_zz(i).t22(1:ID_198), ...
        (daily_epsilon_zz(i).c_dot_delta_t_combo(1:ID_198))+(-1.*daily_epsilon_zz(i).c_dot_delta_t_combo(ID_165)),...
        '-','LineWidth',1.5); hold on;
end
for i=12:1:16
    plot(daily_epsilon_zz(i).t22, ...
...
ylabel('Runoff [cm w.e.] \hspace{6mm} $\dot{c}$  [ m d$^{-1}$ ] \hspace{0mm}','Interpreter','latex','FontSize',fontsize)
set(gca,'ytick',y_plot_cdot(1)+y_plot_cdot_tick:y_plot_cdot_tick:y_plot_cdot(2),'YTickLabel',run_c_dot_label);
text(t_plot(1)+0.15,y_plot_cdot(2)-1.10,'within-basin 1350s','FontName','AvenirBold','FontSize',fontsize+1,'FontAngle','italic','HorizontalAlignment','left')
...
%% print figure
print(gcf,'-dpng','-r300',sprintf('../../paperfigs/suppfig5.2_C5_cdot_distillations_260119.png')); 

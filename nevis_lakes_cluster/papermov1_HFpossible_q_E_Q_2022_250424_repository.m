...
% 24-07-04 LAS: update with lake-typeing dates based on reviewing all S2 images
% 24-08-28 LAS: update with lake-typeing dates based on S1 and S2 :)
...
% 25-06018 LAS: file-path changes for DropBox archive
...
load laketypeing_S1S2_2022_240829.mat % collated from Excel sheet
lake_typeing = laketypeing_S1S2;
type_num = lake_typeing(:,7); % what type of drainage 
lake_num = length(type_num); % number of lakes

% lake locations
load max_extent_indices_GEOD.mat
...
load environs_lakes_2022B_250416.mat
environs_lakes_2022 = environs_lakes; environs_lakes_2022B = environs_lakes; 
X_km = environs_lakes.X_km; Y_km = environs_lakes.Y_km; 
...
%     end
% end
% % redefine lake_typeing to just include lakes within subglacial ROI
...
load daily_lake_HFpossible_classifier_S1S2_2022_240903.mat
% within subglacial ROI:
% for i=1:1:length(environs_lakes_2022.S)
%     if isnan(environs_lakes_2022.S(i))
%         % throw a NaN
...
% load nevis discharge q + E outputs
    addpath('/nevis')
    % load initial timestep
    fn = 'nevis_lakesix_noSK_300m_ub';
...
%% Lake-drainage Type. 2022 Sentinel-2. no change in time. 
figure(1); clf;
...
title('Daily Lake Classification. 2022 Sentinel-2.')
ylabel('N-S [ km ]')
xlabel('E-W [ km ]')
lgd=legend('hydro-fracture','moulin','overspill','water-filled crevasses',...
    'no-exit frozen','slush','stream');
...
% load in one file to get nevis gg structure
for j=1
...
% loop over daily nevis output to calculate domain-wide timeseries
% for j=150:300
%     disp(['Frame ',num2str(j),' / ',num2str(length(j)),' ...']);
%     % load timestep
%     load([fn,'/',int2four(j)]);
...
% % store and save
% nevis_lakesix_2022_noSK_300m_ub_unique.time = time_keep;
% nevis_lakesix_2022_noSK_300m_ub_unique.Q_in = Q_in_keep;
% nevis_lakesix_2022_noSK_300m_ub_unique.Q_out = Q_out_keep;
% nevis_lakesix_2022_noSK_300m_ub_unique.basal_melt = basal_melt_keep;
% nevis_lakesix_2022_noSK_300m_ub_unique.surface_runoff = surface_runoff_keep;
% save nevis_lakesix_2022_noSK_300m_ub_unique.mat nevis_lakesix_2022_noSK_300m_ub_unique
load nevis_lakesix_2022_noSK_300m_ub_unique.mat

%% daily catalogue -- E base, q base, Q timeseries
time_days = 1:1:365;
TriangleSize = 5; 
HF_size = 70;
lake_size = 30;
...
for j=200 %150:1:300 %length(time_days)
%%
Fig2 = figure(j); clf;
...
[surf_contours,surf_labels] = contour(BMv5_for_nevis_catchment_noSK.X_km./1e3, BMv5_for_nevis_catchment_noSK.Y_km./1e3, ...
...
% gps stations
plot(xy_sta_22_short(:,1),xy_sta_22_short(:,2),'k^','MarkerSize',TriangleSize-2,'MarkerFaceColor','k');
...
% HF lake draining
scatter(X_km(1,type_num==1 & Draining==1),Y_km(1,type_num==1 & Draining==1),HF_size+40,...
    'p','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % HF
% moulin lake draining
scatter(X_km(1,type_num==2 & Draining==1),Y_km(1,type_num==2 & Draining==1),lake_size+40,...
    'v','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % moulin
% overspill lake draining
scatter(X_km(1,type_num==3 & Draining==1),Y_km(1,type_num==3 & Draining==1),lake_size+40,...
    'o','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % overspill
% crevasses draining
scatter(X_km(1,type_num==4 & Draining==1),Y_km(1,type_num==4 & Draining==1),lake_size+40,...
...
title(sprintf('2022/%d',time_days(j)), "FontName",'AvenirBold','FontWeight','bold',FontSize=mm+1);
ylabel('North-South [ km ]'); %xlabel('E-W [ km ]')
...
%title(sprintf('2022/%d',time_days(j)), "FontName",'Avenir','FontWeight','bold',FontSize=mm+1);
...
hold on; 
% model output
    plot(nevis_lakesix_2022_noSK_300m_ub_unique.time, nevis_lakesix_2022_noSK_300m_ub_unique.surface_runoff, '-b','LineWidth', 1.3);
    plot(nevis_lakesix_2022_noSK_300m_ub_unique.time, nevis_lakesix_2022_noSK_300m_ub_unique.basal_melt, ':b','LineWidth', 1.3);
    plot(nevis_lakesix_2022_noSK_300m_ub_unique.time, nevis_lakesix_2022_noSK_300m_ub_unique.Q_out, '-','LineWidth', 1.4,'Color',draining_color);
% moving time marker
plot([j j],[1 7999],'-','LineWidth',1.5,'Color',[0.4940 0.1840 0.5560]);
plot(j,8000,'v','MarkerSize',TriangleSize+2,'MarkerFaceColor',[0.4940 0.1840 0.5560],'Color',[0.4940 0.1840 0.5560]);
% model output
    plot(nevis_lakesix_2022_noSK_300m_ub_unique.time, nevis_lakesix_2022_noSK_300m_ub_unique.Q_out, '-','LineWidth', 1.4,'Color',draining_color);
    plot(nevis_lakesix_2022_noSK_300m_ub_unique.time, nevis_lakesix_2022_noSK_300m_ub_unique.surface_runoff, '-b','LineWidth', 1.3);

    ylabel('Q [ m^{3} s^{-1} ]'); xlabel('Day of Year, 2022');

    lgdnames2=[{'Surface Runoff'};{'Basal Melt'};{'Proglacial Discharge'}];
    celllgd2 = cellstr(lgdnames2);

xlim([150 300]); ylim([0 8e3]); 
...
% print figure
%print(gcf,'-dpng','-r250',sprintf('classifier_HFpossible_E_q_base/classifier_2022_HFpossible_E_q_Q_250424_%d.png',time_days(j))); 
%close all
...
% -vcodec libx264 -crf 25 -vf "crop=trunc(iw/2)*2:trunc(ih/2)*2" \
% -pix_fmt yuv420p output-2022-classifier-bedtopo.mp4

%  works better for large images:
% ffmpeg -r 6 -f image2 -pattern_type glob -i "classifier*.png" \
% -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" \
% -vcodec libx264 -crf 20 -pix_fmt yuv420p \
% -profile:v high -level 4.1 -x264opts keyint=6:min-keyint=6:no-scenecut \
% output-2022-classifier.mp4

function [ni_m,sum_m] = nevis_moulins(x_m,y_m,gg,oo,s)
...
% 29 March 2019 LAS: add in elevation cap to moulins
...
if ~isfield(oo,'random_moulins_elevation'), oo.random_moulins_elevation = 0; end % pick random moulin coordinates below an elevation [value should be desired number of moulins]
...
%% pick random moulin coordinates beneath a certain elevation
if oo.random_moulins_elevation
    elev = 800/1000; % cap at some m a.s.l.
    nin = find(s<=elev);
    xl = min(min(gg.nx(nin))); xr = max(max(gg.nx(nin))); 
    yb = min(min(gg.ny(nin))); yt=1.6; %yt = max(max(gg.ny(nin))); 
    n_m = oo.random_moulins_elevation;    %number of moulins
    x_m = xl + (xr-xl)*rand(n_m,1);   %x locations
    y_m = yb + (yt-yb)*rand(n_m,1);   %y locations
%     % or
%     x_m = xl + (xr-xl)*rand(n_m,1).^(1/2);   %x locations

...
ax(ai) = axes('position',axes_size(ai,:));
    ...
    tmp = get(gca,'position'); cbar_size = [tmp(3)*3/16 0.01  tmp(3)*5/8 0.02];
    cx(ai) = colorbar('peer',gca,'horizontal','position',[tmp(1) tmp(2)+tmp(4) 0 0]+cbar_size,'XAxislocation','top');
    tmp = get(cx(ai),'XTick'); labels = 10.^tmp; set(cx(ai),'XTickLabel',labels);
%     text(1.1,0,'m^2 s^{-1}','units','normalized','Parent',cx(ai),'VerticalAlignment','bottom','HorizontalAlignment','left');
    text(-0.05,0,'q [ m^2 s^{-1} ]','units','normalized','Parent',cx(ai),'VerticalAlignment','bottom','HorizontalAlignment','right');

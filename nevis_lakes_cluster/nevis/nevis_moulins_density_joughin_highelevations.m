function [ni_m,sum_m] = nevis_moulins_density_joughin_highelevations(x_m,y_m,gg,oo,aa,ps)
...
% 19 January 2017 : add in moulin density option but only half of L. Andrews values LAS
% 07 February 2017 : add actual moulins and moulin density option for Joughin 2013 paper
...
% L. Andrews Thesis Fig. 2.6 Swiss Camp region moulin densities data 2011
    elev_band = (0:100:2100)';       % Andrews Thesis data through 1100 m a.s.l. 
    p_m = [0, 0, 0.19, 0.22, 0.54, 0.48, 0.35, 0.21, 0.19, 0.16, 0.10, 0.07, 0.04, 0.01, 0.005, 0.001,0.0005,0.0005,0.0005,0.0005,0.0005]'./(1e6); % moulin density in moulin/m2
    p_m = p_m./2; % half the density observed by L. Andrews [ moulin/m2 ]
    p_m = [0, 0, 0.1, 0.15, 0.2415, 0.2415, 0.2019, 0.2469, 0.1193, 0.1042, 0.0614, 0.0165, 0.0165, 0.01, 0.005, 0.005,0.001,0.001,0.0005,0.0005,0.0005]'./(1e6); % moulin density in moulin/m2 for Joughin 2013 region  	
    x_m = []; y_m = [];
...
%% add in Joughin 2013 actual moulin locations inside TerraSARX region
load('nevis/moulin_joughin.mat');
in = inpolygon(x_m, y_m, moulin_joughin.foot(:,1)./10, moulin_joughin.foot(:,2)./10); % moulins in of TerraSARX polygon
x_m2 = x_m(~in); y_m2 = y_m(~in); % select moulins not inside the TerraSARX polygon
x_m = vertcat(x_m2, (moulin_joughin.x./10)'); % add back in Joughin identified moulins
y_m = vertcat(y_m2, (moulin_joughin.y./10)');

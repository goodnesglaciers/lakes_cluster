function nevis_panel9_movie_year_lakesix_2022214_noSK(fn,tis)
...
% 04 MAR 2019: turn into a movie (LAS)

%% options
fn = [oo.root,'nevis_lakesix_noSK_300m_09fw_2'];
...
% set up figure
figure(1); clf; set(gcf,'PaperPositionMode','auto','Units','centimeters','Position',[1 1 200 100]./5);
...
axe3 = axes('Position',[0.01 0.07 0.34 0.25],'Box','on','NextPlot','add'); 
%axe4 = axes('Position',[0.305 0.68 0.35 0.25],'Box','on','NextPlot','add','XTickLabel',[],'YTickLabel',[]); 
axe5 = axes('Position',[-0.005 0.40 0.34+0.34+0.03 0.25+0.25+0.05],'Box','on','NextPlot','add'); 
axe6 = axes('Position',[0.340 0.07 0.34 0.25],'Box','on','NextPlot','add','YTickLabel',[]); 
axe7 = axes('Position',[0.655 0.70 0.34 0.25],'Box','on','NextPlot','add','XTickLabel',[],'YTickLabel',[]); 
...
%     hand = pcolor(xx,yy,zz); set(hand,'linestyle','none'); shading interp;
    ...
    cbar_size = [tmp(1)+tmp(3)-0.02 tmp(2)+tmp(4)*2/8 0.005  tmp(4)*4/8];
    cx1 = colorbar('peer',gca,'vertical','position',cbar_size,'YAxislocation','right');
    text(-54,18,'z [ m ]','VerticalAlignment','bottom','HorizontalAlignment','right','Color','w');
...
%     % color bar
%     tmp = get(gca,'position'); 
%     cbar_size = [tmp(1)+tmp(3)-0.02 tmp(2)+tmp(4)*1/8 0.005  tmp(4)*4/8];
...
axes(axe7);
    ...
    text(-40,18,'E [ mm d^{-1} ]','VerticalAlignment','bottom','HorizontalAlignment','right');
...
%     % color bar
%     tmp = get(gca,'position'); 
%     cbar_size = [tmp(1)+tmp(3)-0.02 tmp(2)+tmp(4)*1/8 0.01 tmp(4)*4/8];
%     cx4 = colorbar('peer',gca,'vertical','position',cbar_size,'YAxislocation','right');
...
% discharge
    ...
    % color bar
    tmp = get(gca,'position'); 
    %cbar_size = [tmp(1)+tmp(3)-0.25 tmp(2)+tmp(4)-0.575 tmp(3)*2/8 0.008];
    cbar_size = [tmp(1)+0.06 tmp(2)+tmp(4)-0.13 tmp(3)*1.25/8 0.008];
    cx5 = colorbar('peer',gca,'horizontal','position',cbar_size,'XAxislocation','bottom');
    tmp = get(cx5,'XTick');  labels = 10.^tmp;  set(cx5,'XTickLabel',labels,'tickdir','out');

    text(-58,22,'q [ m^2 s^{-1} ]','VerticalAlignment','bottom','HorizontalAlignment','right');
    text(-60,16,'\phi [ MPa ]','VerticalAlignment','bottom','HorizontalAlignment','right');
    ylabel('y [ km ]')
    %text(0.5,-0.10/aspect,'x [ km ]','HorizontalAlignment','center','VerticalAlignment','bottom','units','normalized');
    ...
    %text(0.5,-0.13/aspect,'x [ km ]','HorizontalAlignment','center','VerticalAlignment','bottom','units','normalized');
    %text(-0.11,0.5,'y [ km ]','HorizontalAlignment','center','units','normalized','rotation',90);
    set(gca,'TickDir','out','LineWidth',1.01,'TickLength',[0.004 0.004]); box on;
    time = t*10; DOY = time(end);
    title(sprintf('2022/%6.2f',DOY))
    ...
    hold on;
    colormapt=parula(length(x));
    plot(x,y,'k^','Markersize',4,'MarkerFaceColor','none'); % mark GPS; white outline

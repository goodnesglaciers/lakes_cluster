function nevis_panel9_movie_anomaly(fn,tis)
% animates solution in directory fn
%
% 21 August 2014: taken from animate_arolla
% LAS 06 Marck 2020: plot anomalies to pre-drainage values
...
% %% options
% fps = 4;   
fnm = 'movie9-anomaly-take1'; 

%% load initial timestep
if exist([fn,'/0000']), load([fn,'/0000']); else load(fn); end
load('nevis_h22222_ubspatial_R227_lakeflat_4tiles_sigma1e_4_pre.mat');
load([fn,'/0001']);
...
%% load reference model state from DOY 228.8
% load timestep
ref_time = 18; % DOY 288.8
ref_time = 30; % DOY 229.4 % for 9.6 hr floods
ref_time = 22; % DOY 229.0 
load([fn,'/',int2four(ref_time)]);
...
% save with a _ref tag
E_ref = E; % surface input
qs_ref = qs; % discharge
qe_ref = qe;
qQ_ref = qQ; 
phi_0_ref = phi_0; % pressures
phi_ref  = phi;
hs_ref = hs; % sheet height
...
%     end
    ...
    text(35,26.5,('Difference from DOY 229.0'),'FontSize',12,'Interpreter','latex');
...
% discharge anomaly
    xx = (ps.x/10^3)*nx; yy = (ps.x/10^3)*ny;
    zz = ps.qs*reshape(qs+qe+qQ-qs_ref-qe_ref-qQ_ref,gg.nI,gg.nJ);   
    cax = [-0.2 0.2]; 
    
    % non-logarithmic pcolor (logarithmic is used in the non-anomaly case)
    zz = max(cax(1),min(zz,cax(2))); % cap so as to avoid going off color axis
    hand = pcolor(xx,yy,zz); set(hand,'linestyle','none'); % shading interp;
    caxis(cax); 
    axis image;
    ...
    cmap5=load('nevis/cmapbluered.mat'); cmap5=cmap5.cmap; colormap(gca, cmap5(end:-1:1,:));
    cx5 = colorbar('peer',gca,'vertical','position',cbar_size,'YAxislocation','right');
    text(9,22,'\Deltaq [ m^2 s^{-1} ]','VerticalAlignment','bottom','HorizontalAlignment','right');
...
% effective pressure
    xx = (ps.x/10^3)*nx; yy = (ps.x/10^3)*ny;  
    zz = (ps.phi/10^6)*reshape([phi_0-phi-(phi_0_ref-phi_ref)],gg.nI,gg.nJ); 
    cax = [-2 2]; 
    ...
    data = [-2, 0, 2]; % create example data set with values ranging from 0 to 3.6
    ...
    text(8,22,'\DeltaN [ MPa ]','VerticalAlignment','bottom','HorizontalAlignment','right');
...
% sheet thickness
    xx = (ps.x/10^3)*nx; yy = (ps.x/10^3)*ny;  
    zz = (ps.h*100)*reshape(hs-hs_ref,gg.nI,gg.nJ); 
    cax = [-5 5]; 
    ...
    text(7.75,22,'\Deltah_s [ cm ]','VerticalAlignment','bottom','HorizontalAlignment','right');
    
    %load cmapq2; cmap3=cmap; colormap(gca, cmap3);
    cmap3=load('nevis/cmapbluered.mat'); cmap3=cmap3.cmap; colormap(gca, cmap3(end:-1:1,:));
    if reversey, set(gca,'YDir','reverse'); end
...
%     set(gca,'TickDir','out','LineWidth',1.1); box on;
    ...
    load tt_ubspatial_R227_nolake_sigma1e_4_pre.mat % no lake, large sheet permeability
    ...
    % yeslake
    load tt_ubspatial_R227_lakeflat_4tiles_sigma1e_4_pre.mat
    % scaling back to dimensional quantities
...
%     ylim(ax(1), [0 400]); 
    ...
    rectangle('Position',[229.5 1 0.4 497],'EdgeColor','none','FaceColor',[1 0.85 0.85]);
...
axes(axe9)
    ...
    rectangle('Position',[229.5 -0.99 0.4 4.21],'EdgeColor','none','FaceColor',[1 0.85 0.85]);
    plot([doy doy], [-5  5], '-k', 'LineWidth', 1);
    plot([228 240], [0 0], '--k', 'LineWidth', 0.8);
    plot([tt.t].*10,test(1,:),'.-','Color',[0.6 0.6 0.6]);
    plot([tt.t].*10,test(2,:),'.-','Color',colormapt(1,:));
    plot([tt.t].*10,test(3,:),'.-','Color',colormapt(3,:));
    plot([tt.t].*10,test(4,:),'.-','Color',colormapt(4,:));

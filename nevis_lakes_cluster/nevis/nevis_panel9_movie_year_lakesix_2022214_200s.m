function nevis_panel9_movie_year_lakesix_2022214_200s(fn,tis)
...
%% axes limits
% axx = (ps.x/10^3)*[gg.xl gg.xr gg.yb gg.yt]; %entire array
axx = [-10 50 -35 10]; % GNSS full array
axx = [5 35 -26 -1]; % 200s
axx = [20.3997-8 20.3997+8 -13.223-8 -13.223+8]; % 200s centered on SQ24 [20.3997 -13.223]

%% set up figure
figure(1); clf; set(gcf,'PaperPositionMode','auto','Units','centimeters','Position',[1 1 170 100]./5);
set(0,'DefaultAxesFontSize',10);
set(0,'DefaultTextFontSize',10);
axes_size = [0.1 0.1 0.35 0.2];

%axe1 = axes('Position',[-0.01 0.68 0.35 0.25],'Box','on','NextPlot','add','XTickLabel',[]); 
%axe2 = axes('Position',[-0.01 0.37 0.35 0.25],'Box','on','NextPlot','add','XTickLabel',[]); 
axe3 = axes('Position',[-0.01 0.06 0.35 0.25],'Box','on','NextPlot','add'); 
%axe4 = axes('Position',[0.305 0.68 0.35 0.25],'Box','on','NextPlot','add','XTickLabel',[],'YTickLabel',[]); 
axe5 = axes('Position',[-0.045 0.37 0.35+0.35+0.02 0.25+0.25+0.05],'Box','on','NextPlot','add'); 
axe6 = axes('Position',[0.305 0.06 0.35 0.25],'Box','on','NextPlot','add','YTickLabel',[]); 
axe7 = axes('Position',[0.635 0.68 0.35 0.25],'Box','on','NextPlot','add','XTickLabel',[],'YTickLabel',[]); 
axe8 = axes('Position',[0.705 0.375 0.28 0.25],'Box','on','NextPlot','add'); 
axe9 = axes('Position',[0.705 0.06 0.28 0.25],'Box','on','NextPlot','add'); 
...
%     hand = pcolor(xx,yy,zz); set(hand,'linestyle','none'); shading interp;
    ...
    cbar_size = [tmp(1)+tmp(3)-0.04 tmp(2)+tmp(4)*1/8 0.008 tmp(4)*6/8];
    cx1 = colorbar('peer',gca,'vertical','position',cbar_size,'YAxislocation','right');
    text(-45,22,'z [ m ]','VerticalAlignment','bottom','HorizontalAlignment','right');
...
%     text(-45,22,'H [ m ]','VerticalAlignment','bottom','HorizontalAlignment','right');
% 
...
%     text(-35,22,'U_{b} [ m y^{-1} ]','VerticalAlignment','bottom','HorizontalAlignment','right');
%     
%     % add GPS stations
%     if ~isfield(pp,'ni_gps'), pp.ni_gps = []; end
%     mscale = 100;  %amount to scale moulin input by to make size of moulin dot
...
axes(axe7);
    ...
    text(-32,24,'E [ mm d^{-1} ]','VerticalAlignment','bottom','HorizontalAlignment','right');
    %time =  [num2str(round((ps.t/(24*60*60))*t*100)/100)]; 
    time = t*10; DOY = time(end);
    title(sprintf('DOY 2022 %6.2f',DOY))
...
%     % color bar
%     tmp = get(gca,'position'); 
%     cbar_size = [tmp(1)+tmp(3)-0.04 tmp(2)+tmp(4)*1/8 0.01 tmp(4)*6/8];
%     cx4 = colorbar('peer',gca,'vertical','position',cbar_size,'YAxislocation','right');
...
% discharge
    ...
    % color bar
    tmp = get(gca,'position'); 
    cbar_size = [tmp(1)+tmp(3)-0.14 tmp(2)+tmp(4)*2/8 0.008 tmp(4)*3/8];
    cx5 = colorbar('peer',gca,'vertical','position',cbar_size,'YAxislocation','right');
    text(34,-8,'q [ m^2 s^{-1} ]','VerticalAlignment','bottom','HorizontalAlignment','right');
    %text(-58,26,'\phi [ MPa ]','VerticalAlignment','bottom','HorizontalAlignment','right');
    ...
    hold on;
    colormapt=parula(length(x));
    plot(x,y,'w^','Markersize',5,'MarkerFaceColor','none'); % mark GPS; white outline
...
% effective pressure
    ...
    text(-40,22,'N [ MPa ]','VerticalAlignment','bottom','HorizontalAlignment','right');
...
%     text(-42,22,'h_s [ cm ]','VerticalAlignment','bottom','HorizontalAlignment','right');
%     
...
%     ylim(ax(1), [0 400]); 
    ...
    columnlegend(1, celllgd2,'Location','NorthWest'); %,'Position',[-1 232 1 1]);
    
    xlim([140 245]); ylim([0 10000]);
...
axes(axe9)
    ...
    legend(celllgd, 'Location','SouthEast','NumColumns',[4]);
    legend boxoff

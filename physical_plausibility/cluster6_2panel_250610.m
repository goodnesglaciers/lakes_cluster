...
% lake volumes for C6 cluster:
cluster_name = 'C6';
cluster_30s = [208, 201, 228, 238, 273, 275];

% lake volume: option 1 -- linear model of lake surface-area growth:
% volume_30s = environs_lakes_2022.lake_volume_interp(cluster_30s,DOY-1); % m^3 [ linear model]

% lake volume: option 2 -- use maximum surface (pixel) area of the lake:
% use 2022 volume of L3A (ID:232) to avoid ice-lid problem in 2023 (ID:208)
environs_lakes_2022 = load('../GNSS_derived/strain_rates_2022/environs_lakes_2022B_250416.mat'); % drainage dates
environs_lakes_2022 = environs_lakes_2022.environs_lakes;
environs_lakes_2023.max_surface_area(208) = environs_lakes_2022.max_surface_area(232);
...
volume_30s = lake_volume_interp(cluster_30s); % m^3 [ max pixel count of lake ]

ice_thickness_30s = environs_lakes_2023.H(cluster_30s) % m
% all thicknesses are within +/-100 m of 1250m, so use 1.25-km for blister depth 
% blister radius, height, slip for these lakes:
[patches_idealized] = idealized_blister_function(volume_30s,cluster_name); % input volumes in m^3 !!!
% forward Okada [1985] for these lakes, taking H=1.25km:
[sigma_ij_1000m_SLder] = Nsurface_displ_strain_stress_1250m_function(patches_idealized,cluster_name);

%% 200s L2A and L2B
cluster_20s = [142, 132]; % 200s L2A and L2B
...
lake_volume_interp = 3.14159.*((lake_diameter_interp./2).^2).* ...
        ((lake_diameter_interp./100)./3);  % [m^3]
volume_20s = lake_volume_interp(cluster_20s); % m^3 [ max pixel count of lake ]

ice_thickness_20s = environs_lakes_2023.H(cluster_20s); % m
for i=1:length(cluster_20s)
    lake_elevation_20s(i,1) = environs_lakes.S(1,cluster_20s(i));
    lake_position_20s(i,1:2) = [environs_lakes.X_km(1,cluster_20s(i)), environs_lakes.Y_km(1,cluster_20s(i))];
   % flow angle
    theta_20s(i,1) = flow_angle((ID_stresses_sig1_winter(cluster_20s(i),1))); % [ deg ]
end

%% create map of ROI with these elastic stresses in the right place
% origins for each lake:
origin_cluster = [environs_lakes_2023.X_km(cluster_30s); environs_lakes_2023.Y_km(cluster_30s)];
% blank X-Y grid grid points = nevis xx yy
xx_vec = reshape(xx,601*264,1); 
yy_vec = reshape(yy,601*264,1);
for i=1:length(cluster_30s)
    % flow angle
    theta = flow_angle((ID_stresses_sig1_winter(cluster_30s(i),1))); % [ deg ]
    % rotation matrix
...
theta_range = 140:1:240; % degrees with East = 0 degrees. --> 250–310 in ice-flow direction
...
for i=1:length(cluster_30s); for j=1:length(time_range)
    % circle xy-points
...
% 30s
cluster_30s = [208, 201, 228, 238, 273, 275];
cc2 = plot([environs_lakes_2023.X_km(cluster_30s(1)) environs_lakes_2023.X_km(cluster_30s(2))],...
    [environs_lakes_2023.Y_km(cluster_30s(1)) environs_lakes_2023.Y_km(cluster_30s(2))],...
    '-','LineWidth',2.5,'Color',grey_connections);
cc21 =plot([environs_lakes_2023.X_km(cluster_30s(2)) environs_lakes_2023.X_km(cluster_30s(3))],...
    [environs_lakes_2023.Y_km(cluster_30s(2)) environs_lakes_2023.Y_km(cluster_30s(3))],...
    '-','LineWidth',2.5,'Color',grey_connections);
cc22 =plot([environs_lakes_2023.X_km(cluster_30s(3)) environs_lakes_2023.X_km(cluster_30s(4))],...
    [environs_lakes_2023.Y_km(cluster_30s(3)) environs_lakes_2023.Y_km(cluster_30s(4))],...
    '-','LineWidth',2.5,'Color',grey_connections);
% cc23 =plot([environs_lakes_2023.X_km(cluster_30s(1)) environs_lakes_2023.X_km(cluster_30s(5))],...
%     [environs_lakes_2023.Y_km(cluster_30s(1)) environs_lakes_2023.Y_km(cluster_30s(5))],...
%     '-','LineWidth',2.5,'Color',grey_connections);
cc24 =plot([environs_lakes_2023.X_km(cluster_30s(6)) environs_lakes_2023.X_km(cluster_30s(5))],...
    [environs_lakes_2023.Y_km(cluster_30s(6)) environs_lakes_2023.Y_km(cluster_30s(5))],...
    '-','LineWidth',2.5,'Color',grey_connections);
% lake identification number labels
text(environs_lakes_2023.X_km(cluster_30s(1))+1, environs_lakes_2023.Y_km(cluster_30s(1))+0.25,...
    "L208","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_30s(1))+4, environs_lakes_2023.Y_km(cluster_30s(1))+2.75,...
    "(L3A)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_30s(2))-3.0, environs_lakes_2023.Y_km(cluster_30s(2))-2.25,...
    "L201","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_30s(2))-6.0, environs_lakes_2023.Y_km(cluster_30s(2))-4.5,...
    "(L3B)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_30s(3))+1.25, environs_lakes_2023.Y_km(cluster_30s(3))+0.5,...
    "L228(L3C)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_30s(4))+1.25, environs_lakes_2023.Y_km(cluster_30s(4))+0,...
    "L238 (L3D)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_30s(5))+1.25, environs_lakes_2023.Y_km(cluster_30s(5))+0.75,...
    "L273 (L3E)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_30s(6))+1, environs_lakes_2023.Y_km(cluster_30s(6)),...
    "L275 (L3F)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
...
scatter3(X_km(1,type_num==1 & Empty==1),Y_km(1,type_num==1 & Empty==1),...
    1e10.*X_km(1,type_num==1 & Empty==1),HF_size,...
...
% HF lake draining
scatter3(X_km(1,type_num==1 & Draining==1),Y_km(1,type_num==1 & Draining==1),...
    1e10.*X_km(1,type_num==1 & Draining==1),HF_size+40,...
    'p','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % HF
% moulin lake draining
scatter(X_km(1,type_num==2 & Draining==1),Y_km(1,type_num==2 & Draining==1),lake_size,...
    'v','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % moulin
...
lgdB = legend('\sigma_{1}','\sigma_{1} = 200 kPa',...
    '24-hr propagation');   
set(lgdB,'Position',[0.875 0.439 0.05 0.04],'FontSize',fontsize-2,'FontName','Avenir',...
    'NumColumns',1,'Box','on','TextColor',lake_blue(1:3))
...
%% print figure
print(gcf,'-dpng','-r300',sprintf('../paperfigs/suppfig6.1_%s_260129.png',cluster_name)); 
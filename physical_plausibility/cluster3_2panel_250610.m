...
% lake volumes for C3 cluster:
cluster_name = 'C3';
cluster_20s = [143, 155, 156, 187]; % L2A, L2D, L2B, L2C

% lake volume: option 1 -- linear model of lake surface-area growth:
% volume_20s = environs_lakes_2022.lake_volume_interp(cluster_20s,DOY-1); % m^3
% volume_20s(1) = 0.027*(1e9); volume_20s(3) = 0.009*(1e9); % m3
...
ice_thickness_20s = environs_lakes_2022.H(cluster_20s) % m
% all thicknesses are within +/-75 m of 1250m, so use 1.25-km for blister depth 
% blister radius, height, slip for these lakes:
[patches_idealized] = idealized_blister_function(volume_20s,cluster_name); % input volumes in m^3 !!!
...
origin_cluster = [environs_lakes_2022.X_km(cluster_20s); environs_lakes_2022.Y_km(cluster_20s)];
% blank X-Y grid grid points = nevis xx yy
xx_vec = reshape(xx,601*264,1); 
yy_vec = reshape(yy,601*264,1);
for i=1:length(cluster_20s)
    % flow angle
    theta(i,1) = flow_angle((ID_stresses_sig1_winter(cluster_20s(i),1))); % [ deg ]
    % rotation matrix
...
theta_range = 140:1:220; % degrees with East = 0 degrees. --> 230 to“310 in ice-flow direction
...
for i=1:length(cluster_20s); for j=1:length(time_range)
    % circle xy-points
...
% 20s
cluster_20s = [143, 155, 156, 187];  % L2A, L2D, L2B, L2C
cluster_mid = cluster_20s;
cc2 = plot([environs_lakes_2022.X_km(cluster_mid(1)) environs_lakes_2022.X_km(cluster_mid(3))],...
    [environs_lakes_2022.Y_km(cluster_mid(1)) environs_lakes_2022.Y_km(cluster_mid(3))],...
    '-','LineWidth',2,'Color',grey_connections);
cc21 =plot([environs_lakes_2022.X_km(cluster_mid(2)) environs_lakes_2022.X_km(cluster_mid(3))],...
    [environs_lakes_2022.Y_km(cluster_mid(2)) environs_lakes_2022.Y_km(cluster_mid(3))],...
    ':','LineWidth',2,'Color',grey_connections);
cc22 =plot([environs_lakes_2022.X_km(cluster_mid(1)) environs_lakes_2022.X_km(cluster_mid(4))],...
    [environs_lakes_2022.Y_km(cluster_mid(1)) environs_lakes_2022.Y_km(cluster_mid(4))],...
    '-','LineWidth',2,'Color',grey_connections);
cc23 =plot([environs_lakes_2022.X_km(cluster_mid(4)) environs_lakes_2022.X_km(cluster_mid(3))],...
    [environs_lakes_2022.Y_km(cluster_mid(4)) environs_lakes_2022.Y_km(cluster_mid(3))],...
    '-','LineWidth',2,'Color',grey_connections);
% lake identification number labels
text(environs_lakes_2022.X_km(cluster_mid(1))-7, environs_lakes_2022.Y_km(cluster_mid(1))+0.5,...
    "L143","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2022.X_km(cluster_mid(1))-9, environs_lakes_2022.Y_km(cluster_mid(1))-2.0,...
    "(L2B)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2022.X_km(cluster_mid(2))+1, environs_lakes_2022.Y_km(cluster_mid(2))-0.5,...
    "L155 (L2D)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2022.X_km(cluster_mid(3))+1.25, environs_lakes_2022.Y_km(cluster_mid(3))+0.25,...
    "L156","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2022.X_km(cluster_mid(3))+7.25, environs_lakes_2022.Y_km(cluster_mid(3))+0.75,...
    "(L2A)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2022.X_km(cluster_mid(4))+0.25, environs_lakes_2022.Y_km(cluster_mid(4))-2,...
    "L187","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2022.X_km(cluster_mid(4))+0.25, environs_lakes_2022.Y_km(cluster_mid(4))-4.5,...
    "(L2C)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
...
hold all
% elastic blister sigma1 + winter sigma1 + bound viscous
...
cc21 =plot([environs_lakes_2022.X_km(cluster_mid(2)) environs_lakes_2022.X_km(cluster_mid(3))],...
    [environs_lakes_2022.Y_km(cluster_mid(2)) environs_lakes_2022.Y_km(cluster_mid(3))],...
    ':','LineWidth',2,'Color',grey_connections); % deemed implausible
cc22 =plot([environs_lakes_2022.X_km(cluster_mid(1)) environs_lakes_2022.X_km(cluster_mid(4))],...
    [environs_lakes_2022.Y_km(cluster_mid(1)) environs_lakes_2022.Y_km(cluster_mid(4))],...
    '-','LineWidth',2,'Color',grey_connections);
cc23 =plot([environs_lakes_2022.X_km(cluster_mid(4)) environs_lakes_2022.X_km(cluster_mid(3))],...
...
%% print figure
print(gcf,'-dpng','-r300',sprintf('../paperfigs/paperfig8_%s_260129.png',cluster_name)); 
print(gcf,'-dpng','-r300',sprintf('../paperfigs/suppfig3.1_%s_260129.png',cluster_name)); 
...
% lake volumes for C4 cluster:
cluster_name = 'C4';
cluster_10s = [44, 56, 66, 75, 81, 86, 103, 131, 134];
cluster_10s = [44, 66, 75, 81, 86, 103, 131, 134]; % 56 happens on 190

% lake volume: option 1 -- linear model of lake surface-area growth:
% volume_10s = environs_lakes_2022.lake_volume_interp(cluster_10s,DOY-1); % m^3 [ linear model]
...
lake_diameter_interp = 2.*sqrt((environs_lakes_2023.max_surface_area.*1e6)./3.14159); % average diameter of this lake [m]
...
volume_10s = lake_volume_interp(cluster_10s); % m^3 [ max pixel count of lake ]

ice_thickness_10s = environs_lakes_2023.H(cluster_10s) % m
% all thicknesses are within +/-75 m of 1000m, so use 1.00-km for blister depth 
% blister radius, height, slip for these lakes:
[patches_idealized] = idealized_blister_function(volume_10s,cluster_name); % input volumes in m^3 !!!
% forward Okada [1985] for these lakes, taking H=1km:
[sigma_ij_1000m_SLder] = Nsurface_displ_strain_stress_1000m_function(patches_idealized,cluster_name);

%% create map of ROI with these elastic stresses in the right place
% origins for each lake:
origin_cluster = [environs_lakes_2023.X_km(cluster_10s); environs_lakes_2023.Y_km(cluster_10s)];
% blank X-Y grid grid points = nevis xx yy
xx_vec = reshape(xx,601*264,1); 
yy_vec = reshape(yy,601*264,1);
for i=1:length(cluster_10s)
    % flow angle
    theta = flow_angle((ID_stresses_sig1_winter(cluster_10s(i),1))); % [ deg ]
    % rotation matrix
    Rotation=[cosd(theta) -sind(theta); sind(theta) cosd(theta)]; % create rotation matrix
    % align with ice-flow direction by rotating grid
    rotXY = Rotation*[sigma_ij_1000m_SLder.xx(:).'; sigma_ij_1000m_SLder.yy(:).']; % vectors X rotation matrix 
    XX = reshape(rotXY(1, :), size(sigma_ij_1000m_SLder.xx));
    YY = reshape(rotXY(2, :), size(sigma_ij_1000m_SLder.yy));
    ...
    nevis_sigma_xx(i,:) = griddata(x_sigma,y_sigma,sigma_ij_1000m_SLder.sigma_xx(i,:),xx_vec,yy_vec); % xx
    nevis_sigma_yy(i,:) = griddata(x_sigma,y_sigma,sigma_ij_1000m_SLder.sigma_yy(i,:),xx_vec,yy_vec); % yy
    nevis_sigma_xy(i,:) = griddata(x_sigma,y_sigma,sigma_ij_1000m_SLder.sigma_xy(i,:),xx_vec,yy_vec); % xy
end
...
theta_range = 160:1:250; % degrees with East = 0 degrees. --> 200–290 in ice-flow direction
...
for i=1:length(cluster_10s); for j=1:length(time_range)
    % circle xy-points
...
%% how many viable lakes are there?
% within the ROI
viable = daily_lake_classifier(:,DOY)~=0 & isfinite(daily_lake_classifier(:,DOY)) & ...
    daily_lake_classifier(:,DOY)~=4;
...
% nevis discharge: plot frames
    ...
    axis image;
    % color bar
    cbar_size = [0.76 0.546 0.165 0.005];
...
% main array GNSS
plot3(xy_sta_23_short(:,1),xy_sta_23_short(:,2),300.*ones(num_sta_23,1),'k^',...
...
cc1 = plot([environs_lakes_2023.X_km(cluster_10s(1)) environs_lakes_2023.X_km(cluster_10s(3))],...
    [environs_lakes_2023.Y_km(cluster_10s(1)) environs_lakes_2023.Y_km(cluster_10s(3))],...
    '-','LineWidth',2,'Color',grey_connections);
cc12 =plot([environs_lakes_2023.X_km(cluster_10s(2)) 0],...
    [environs_lakes_2023.Y_km(cluster_10s(2)) 4],...
    '-','LineWidth',2,'Color',grey_connections);
cc13 =plot([environs_lakes_2023.X_km(cluster_10s(3)) environs_lakes_2023.X_km(cluster_10s(4))],...
    [environs_lakes_2023.Y_km(cluster_10s(3)) environs_lakes_2023.Y_km(cluster_10s(4))],...
    '-','LineWidth',2,'Color',grey_connections);
cc13 =plot([environs_lakes_2023.X_km(cluster_10s(5)) environs_lakes_2023.X_km(cluster_10s(4))],...
    [environs_lakes_2023.Y_km(cluster_10s(5)) environs_lakes_2023.Y_km(cluster_10s(4))],...
    '-','LineWidth',2,'Color',grey_connections);
cc13 =plot([environs_lakes_2023.X_km(cluster_10s(3)) environs_lakes_2023.X_km(cluster_10s(6))],...
    [environs_lakes_2023.Y_km(cluster_10s(3)) environs_lakes_2023.Y_km(cluster_10s(6))],...
    ':','LineWidth',2,'Color',grey_connections);
cc13 =plot([environs_lakes_2023.X_km(cluster_10s(5)) environs_lakes_2023.X_km(cluster_10s(7))],...
    [environs_lakes_2023.Y_km(cluster_10s(5)) environs_lakes_2023.Y_km(cluster_10s(7))],...
    '-','LineWidth',2,'Color',grey_connections);
cc13 =plot([environs_lakes_2023.X_km(cluster_10s(8)) environs_lakes_2023.X_km(cluster_10s(7))],...
    [environs_lakes_2023.Y_km(cluster_10s(8)) environs_lakes_2023.Y_km(cluster_10s(7))],...
    '-','LineWidth',2,'Color',grey_connections);
cc13 =plot([environs_lakes_2023.X_km(cluster_10s(5)) 0.1],...
    [environs_lakes_2023.Y_km(cluster_10s(5)) 4.0],...
    '-','LineWidth',2,'Color',grey_connections); %L1C
scatter(0.1,4,HF_size+40,...
    'p','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % HF L1C
% lake identification number labels
text(environs_lakes_2023.X_km(cluster_10s(1))+0.5, environs_lakes_2023.Y_km(cluster_10s(1))-2,...
    "L44","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(2))-0.5, environs_lakes_2023.Y_km(cluster_10s(2))+2.25,...
    "L66","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(3))+1, environs_lakes_2023.Y_km(cluster_10s(3)),...
    "L75","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(4))+1, environs_lakes_2023.Y_km(cluster_10s(4))-0.75,...
    "L81","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(4))-0.75, environs_lakes_2023.Y_km(cluster_10s(4))-2.75,...
    "(L1B)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(5))+1, environs_lakes_2023.Y_km(cluster_10s(5))+1.5,...
    "L86","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(5))+5.5, environs_lakes_2023.Y_km(cluster_10s(5))+2.0,...
    "(L1A)","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(6))+1, environs_lakes_2023.Y_km(cluster_10s(6)),...
    "L103","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(7))+1, environs_lakes_2023.Y_km(cluster_10s(7)),...
    "L131","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(environs_lakes_2023.X_km(cluster_10s(8))+1, environs_lakes_2023.Y_km(cluster_10s(8)),...
    "L134","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
text(0.1-1.5, 4+3,...
    "L1C","Color",'k',"FontSize",fontsize,"FontName","Helvetical",'FontWeight','Bold','FontAngle','italic')
...
scatter3(X_km(1,type_num==1 & Draining==1),Y_km(1,type_num==1 & Draining==1),...
    1e50+abs(X_km(1,type_num==1 & Draining==1)),HF_size+40,...
    'p','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % HF
scatter3(0.1,4,1e50,HF_size+40,...
    'p','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % HF L1C
...
scatter3(X_km(1,type_num==1 & Empty==1),Y_km(1,type_num==1 & Empty==1),...
    4e10.*abs(X_km(1,type_num==1 & Empty==1)),HF_size,...
...
lgdA = legend('Subglacial Discharge {\itq}','Ice Elevation','Subglacial ROI',...
    'GNSS station','C4 HF events');
set(lgdA,'Position',[0.875 0.910 0.05 0.05],'FontSize',fontsize-2,'FontName','Avenir',...
    'NumColumns',1,'Box','on','TextColor',lake_blue(1:3))

axis equal; grid on;
date_of_q = sprintf('2023/%d',DOY);
text(-33, 15, date_of_q,'FontSize',fontsize+4,'FontName','Helvetica','FontWeight','bold')
...
% sig1 map ELASTIC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
axes(axeB) % sigma1_elastic
xlabel('East-West [ km ]','FontSize',fontsize);
ylabel('North-South [ km ]','FontSize',fontsize);
set(gca,'xaxislocation','bottom','FontSize',fontsize)
text(-41, 16, 'b','FontWeight','bold','FontSize',fontsize+3);
set(gcf, 'color', [1 1 1])

hold all
% elastic blister sigma1
...
% color bar
    ...
    cbar_size = [0.760 0.062 0.165 0.005];
...
% CLASSIFIER PLOTTING
% lake filling
Filling = daily_lake_classifier(:,DOY+1)==1;
% HF lake filling
scatter(X_km(1,type_num==1 & Filling==1),Y_km(1,type_num==1 & Filling==1),HF_size,...
    'p','filled','MarkerFaceColor',filling_color,'MarkerEdgeColor','k','LineWidth',0.8); % HF
% moulin lake filling
...
% lake draining -- initial
Draining = daily_lake_classifier(:,DOY+1)==2;
% HF lake draining
scatter3(X_km(1,type_num==1 & Draining==1),Y_km(1,type_num==1 & Draining==1),...
    4e10+abs(X_km(1,type_num==1 & Draining==1)),HF_size+40,...
    'p','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % HF
% moulin lake draining
scatter(X_km(1,type_num==2 & Draining==1),Y_km(1,type_num==2 & Draining==1),lake_size,...
    'v','filled','MarkerFaceColor',draining_color,'MarkerEdgeColor','k'); % moulin
...
% lake draining -- continued
Still_Draining = daily_lake_classifier(:,DOY+1)==3;
% moulin lake draining
scatter(X_km(1,type_num==2 & Still_Draining==1),Y_km(1,type_num==2 & Still_Draining==1),lake_size,...
    'v','filled','MarkerFaceColor',still_draining_color,'MarkerEdgeColor','k'); % moulin
% overspill lake draining
...
% HF empty basin
Empty = daily_lake_classifier(:,DOY+1)==3;
% HF empty basin
scatter3(X_km(1,type_num==1 & Empty==1),Y_km(1,type_num==1 & Empty==1),...
    4e10+abs(X_km(1,type_num==1 & Empty==1)),HF_size,...
    'p','filled','MarkerFaceColor',frozen_color,'MarkerEdgeColor','k','LineWidth',0.8); % HF
...
% lake frozen
Frozen = daily_lake_classifier(:,DOY+1)==4;
% moulin lake frozen
scatter(X_km(1,type_num==2 & Frozen==1),Y_km(1,type_num==2 & Frozen==1),lake_size,...
    'v','filled','MarkerFaceColor',frozen_color,'MarkerEdgeColor','k','LineWidth',0.8); % moulin
% overspill lake frozen
...
% propagating blister speed radar circles 24-hr:
for i=1:length(cluster_10s)
plot(squeeze(x_circle(i,2,:)),squeeze(y_circle(i,2,:)),'-','LineWidth',2,'Color',[0.6 0.6 0.6]); 
end

% cluster cOneXiOnS
grey_connections = [0 0 0]; %lake_blue(1:3).*1.3; %[0.7 0.7 0.7];
% 10s
cluster_10s = [44, 66, 75, 81, 86, 103, 131, 134];
cc1 = plot([environs_lakes_2023.X_km(cluster_10s(1)) environs_lakes_2023.X_km(cluster_10s(3))],...
    [environs_lakes_2023.Y_km(cluster_10s(1)) environs_lakes_2023.Y_km(cluster_10s(3))],...
...
cc13 =plot([environs_lakes_2023.X_km(cluster_10s(5)) 0.1],...
    [environs_lakes_2023.Y_km(cluster_10s(5)) 4.0],...
    '-','LineWidth',2,'Color',grey_connections); %L1C
scatter3(0.1,4,1e10,HF_size,...
    'p','filled','MarkerFaceColor',frozen_color,'MarkerEdgeColor','k'); % HF L1C

lgdB = legend('\sigma_{1}','\sigma_{1} = 200 kPa',...
    '24-hr propagation',...
...
set(lgdB,'Position',[0.875 0.305 0.05 0.04],'FontSize',fontsize-2,'FontName','Avenir',...
    'NumColumns',1,'Box','on','TextColor',lake_blue(1:3))

axis equal; grid on;
date_of_q = sprintf('2023/%d',DOY+1);
text(-33, 15, date_of_q,'FontSize',fontsize+4,'FontName','Helvetica','FontWeight','bold')
set(gca,'FontName','Avenir','FontSize',fontsize,'xtick',-80:10:110,...
    'ytick',-50:10:30,'LineWidth',0.6,'Layer','top')
xlim([plot_x(1) plot_x(2)]); ylim([plot_y(1) plot_y(2)]);
...
%% print figure
print(gcf,'-dpng','-r300',sprintf('../paperfigs/suppfig4.1_%s_260129.png',cluster_name)); 